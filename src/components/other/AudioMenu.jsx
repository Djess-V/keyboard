...
export default function AudioMenu({ pressedButtonUpDown, audioOnClick = f => f, raiseAudio = f => f }) {
  const [songActive, setSongActive] = React.useState(0);
  const lis = audioList.map((item, i) => (
    <li className={`audio_list_item ${(songActive === i) ? "songActive" : ""}`} key={item.name}>
      <a href={item.link}  onClick={(e) => {e.preventDefault(); audioOnClick(e.target.href); }}>{item.name}</a>
    </li>
  ));
